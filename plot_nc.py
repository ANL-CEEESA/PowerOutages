...
def plot_outage_and_weather_linechart(titlename, start_date, 
    ...
    # ground
    ground1 = np.zeros(N) + 5000
    ground2 = np.zeros(N)
    ground3 = np.ones(N) * 100.

    # outage data
    line_outage    = obs_outage.sum(1) / 3 * 2 + 5000
    ...
    ax1.set_yticks([5000, 10000, 15000])
    ax1.set_yticklabels([0, 7500, 15000], fontproperties)
    ax1.set_ylim(-600, 15000)
    ...
    fig.tight_layout()
    fig.savefig("imgs/outage-vs-weather-line-ncsc.pdf")

def plot_data_on_map_in_color(data, locs, filename="recoveryrate-ncsc"):
    ...
    if filename == "recoveryrate-ncsc":
        plt.title(r"Recovery rate $\beta$ in NC \& SC")
    else:
        plt.title(r"Vulnerability to extreme weather $\gamma$ in NC \& SC")
    fig.tight_layout()
    plt.savefig("imgs/%s.pdf" % filename)
...
def plot_baselines_and_lambdas(model, start_date, obs_outage, dayinterval=3):
    ...
    # plt.legend(loc="upper right")
    plt.title("Hurricane Isaias in August 2020, NC \& SC")
    fig.tight_layout()
