...
from dataloader import concise_new_feat_list
from utils import avg, truncate_colormap
...
def plot_interval_qqplot(obs_outage, start_date, filename, dayinterval=3, vmax=None, vmin=None):
    ...
    plt.xticks(np.arange(0, N, int(4 * 24 * dayinterval)), [ str(date.format('MMM DD')) for date in dates ], rotation=90)
    plt.yticks(np.arange(0, N, int(4 * 24 * dayinterval)), [ str(date.format('MMM DD')) for date in dates ])
    fig.tight_layout()
...
def plot_outage_and_weather_map(outage_locs, weather_locs, obs_outage, obs_weather_show, obs_weather_normal):
    ...
    weather_ratio = obs_weather_show / obs_weather_normal * 100
    inds          = np.where(weather_ratio > 100.)[0]
    print(inds)
    print(weather_ratio.max())

    cm  = plt.cm.get_cmap('Reds')
    sct = m.scatter(weather_locs[inds, 1], weather_locs[inds, 0], latlon=True, alpha=0.5, s=8, c=weather_ratio[inds], cmap=cm, vmin=100, vmax=weather_ratio.max())
    # handles, labels = sct.legend_elements(prop="sizes", alpha=0.6, num=4, 
    #     func=lambda s: (s - mins) / (maxs - mins) * (dmax - dmin) + dmin)
    # plt.title(filename)
    # plt.legend(handles, labels, loc="lower left", title="Num of outages")

    plt.savefig("imgs/outage-vs-weather-map.png")

def plot_outage_and_weather_linechart(titlename, start_date, 
    obs_outage, obs_weather_show, obs_weather_normal, 
    labels, colors, dayinterval=3):

    obs_outage     = obs_outage / 15
    weather_show   = obs_weather_show.sum(2)
    weather_normal = obs_weather_normal.sum(2)
    
    N = obs_outage.shape[0]
    ...
    # ground
    ground1 = np.zeros(N) + 4333
    ground2 = np.zeros(N)
    ground3 = np.ones(N) * 100.

    # outage data
    line_outage    = obs_outage.sum(1) / 3 * 2 + 4333
    ...
    ax1.set_yticks([4333, 8666, 13000])
    ax1.set_yticklabels([0, 6500, 13000], fontproperties)
    ax1.set_ylim(-520, 13000)
    ax1.set_xlabel(r"Dates")
    ax1.set_ylabel(r"Number per minute")

    ax2.set_yticks([0, 50, 100, 150, 200, 250])
    ax2.set_yticklabels([0, 50, 100, 150, 200, 250], fontproperties)
    ax2.set_ylim(-12, 300)
    ...
    plt.title(titlename)
    plt.xticks(np.arange(0, N, int(24 * dayinterval)), dates) 
    fig.tight_layout()
    fig.savefig("imgs/outage-vs-weather-line-ga.pdf")

def plot_data_on_map_in_color(data, locs, filename="recoveryrate-ga"):
    ...
    if filename == "recoveryrate-ga":
        plt.title(r"Recovery rate $\beta$ in GA")
    else:
        plt.title(r"Vulnerability to extreme weather $\gamma$ in GA")
    fig.tight_layout()
    plt.savefig("imgs/%s.pdf" % filename)
...
def plot_baselines_and_lambdas(model, start_date, obs_outage, dayinterval=3):
    ...
    # plt.legend(loc="upper right")
    plt.title("Hurricane Michael in October 2018, GA")
    fig.tight_layout()
